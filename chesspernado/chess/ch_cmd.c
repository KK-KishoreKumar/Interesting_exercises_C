...
int command_check(BOARD (*board)[SIZE], char *s) {
	...
	switch(piece) {
		...
		case 'R':	if(!rook_check(board, i1, j1, i2, j2)) return 0; break;
		case 'Q':	if(!queen_check(board, i1, j1, i2, j2)) return 0; break;
	}
...
int king_check(BOARD (*board)[SIZE], int i1, int j1, int i2, int j2) {
	...
	if (!check && i1 == i2 && absj == 2 && !board[i1][j1].state) {
		if(j1 < j2) {
			if(board[i1][SIZE-1].type == 'R' && !board[i1][SIZE-1].state 
			...
			if(board[i1][0].type == 'R' && !board[i1][0].state
...
int syntax_check(char *p) {
	if(*p != 'N' && *p != 'P' && *p != 'Q' && *p != 'B' && *p != 'K' && *p != 'R') return 0;
	if(*++p < 'a' || *p > 'h') return 0;
	if(*++p < '1' || *p > '8') return 0;
	if(*++p < 'a' || *p > 'h') return 0;
	if(*++p < '1' || *p > '8') return 0;
