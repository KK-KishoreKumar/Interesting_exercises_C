		...
		for(j = 0; j < SIZE; j++) {
			if(board[i][j].player == player) {
				switch(board[i][j].type) {
					...
					case 'N': if(can_smov(board, i, j, player)) return 0; break;
					case 'B': if(can_bmov(board, i, j, player)) return 0; break;
					case 'R': if(can_rmov(board, i, j, player)) return 0; break;
					case 'Q': if(can_bmov(board, i, j, player) || can_rmov(board, i, j, player)) return 0; break;
				}
			}
		}
	}
